...
class MLA {
	...
	public static function initialize( )
	{
		...
		add_action( 'admin_menu', 'MLA::add_admin_menu' );
		add_action( 'admin_enqueue_scripts', 'MLA::mla_admin_enqueue_scripts_action' );
		add_action( 'admin_enqueue_scripts', 'MLA::admin_enqueues' );
		add_action( 'admin_menu', 'MLA::mla_admin_menu_action' );
		//ajax actions
		add_action( 'wp_ajax_regeneratethumbnail', 'MLA::ajax_process_image' );
		add_action( 'wp_ajax_save_av_upload', 'MLA::save_av_upload' );
		add_action( 'wp_ajax_refresh_media', 'MLA::refresh_media' );
		//screen options
	...
	public static $capability = false;
	public static $menuId = false;

	    // Register the management page
	
	public static function add_admin_menu() {
	
	if ( self::$capability == false )
			self::$capability = apply_filters( 'regenerate_thumbs_cap', 'manage_options' );
	if ( self::$menuId == false )
		self::$menuId = add_management_page( __( 'Regenerate Thumbnails', 'regenerate-thumbnails' ), __( 'Regen. Thumbnails', 'regenerate-thumbnails' ), self::$capability, 'regenerate-thumbnails', 'MLA::regenerate_interface' );
		
		//adds transloadit uploader for audio/video files
		add_media_page( 'Add Audio/Video', 'Add Audio/Video', 'upload-files', 'avuploader', 'MLA::addAVUploader');
	
	}
	
	/**
	 * Adds an Audio/Video uploader with transcoding capabilities
	 * as well as thumbnail generation.
	 *
	 *
	 **/
	public static function addAVUploader() {
	
		if (!current_user_can('upload_files'))
			wp_die(__('You do not have permission to upload files.'));
		
		$ampWidgets = AMPWidgets::get_instance();
        $ampWidgets->includeQTip();
        $ampWidgets->includeAmpMedia();
        $_plugin_slug = MLA::STYLESHEET_SLUG;
        wp_enqueue_script( 'upload-av-scripts', plugins_url( 'assets/js/upload-av.js', __FILE__ ), array( 'jquery' ), self::CURRENT_MLA_VERSION );
		
		include_once('views/avuploader.php');
	
	}
	
	/**
	 * Takes data back via ajax from TMS and creates an attachment in the media library using the source url and metadata
	 * associated with the attachment.
	 * 
	 *
	 **/
	 
	public static function save_av_upload() {
        
        if ( empty($_POST['upload']) ) 
            die('<p class="message error">Missing Upload ID</p>');
        
        $upload  = intval( $_POST['upload'] );
        
        // Get the nitty gritty for outputting
        $media = AMPMedia::getMediaDetail( $upload, true );
        
        error_log( "media data: " . print_r( $media, true ) );

        // Save to META
		update_post_meta( $upload, 'media', $media );
		
		
		//get info from transloadit
		$transData = get_post_meta ( $upload, 'transloadit', true );
		error_log( "transloadit info: " . print_r( $transData, true ) );
		
		$tms_media_id = AMPTms::uploadMediaUrl( $transData['url'], $transData['name'] );
		
		error_log( "tms_media_id: " . print_r( $tms_media_id, true ) );
        
        include_once('views/refresh.avuploader.php');
	...
	/**
     * AJAX handler to refresh a bit of media for the 
     */
    public static function refresh_media() {
        ...
        $tms_media_id = get_post_meta( $post_id, 'tms_media_id', true );
        
        // Get info from TMS and update the DB post_meta for TMS
        AMPTms::refreshMediaDetail($post_id);
	...
	public static function mla_render_admin_page( ) {
        ...
        // Check for an export bulk action request
        if ( $bulk_action && $bulk_action == 'export' && isset( $_REQUEST['cb_attachment'] ) ) {

            // Which media item(s) are being requested for export?
            $media = $_REQUEST['cb_attachment'];
            if ( is_numeric( $media ) ) {
                $media = array( (int)$media );
            } else if ( $media == 'all' ) {
                $media = array();
            ...
            // Package and serve the export archive
            $item_content = self::_export_items( $media );

            // We've fulfilled the request, so bail out
            die();
        }
        
        if ( $bulk_action && $bulk_action == 'bulk_regenerate_thumbnails' && isset( $_REQUEST['cb_attachment'] ) ) {
        
        	if ( empty( $_REQUEST['action'] ) || ( 'bulk_regenerate_thumbnails' != $_REQUEST['action'] && 'bulk_regenerate_thumbnails' != $_REQUEST['action2'] ) )
				return;

			if ( empty( $_REQUEST['cb_attachment'] ) || ! is_array( $_REQUEST['cb_attachment'] ) )
				return;
      
			// Handy check to make sure this came from... this WP_List_Table (I suppose...)
			//check_admin_referer( 'bulk-media' );
      
      
			// Determine Affected Media IDs
			$ids = implode( ',', array_map( 'intval', $_REQUEST['cb_attachment'] ) );
...
//			echo '<form action="' . admin_url( 'upload.php' ) . '" method="get" id="mla-filter">' . "\r\n";
			echo '<form action="' . admin_url( 'upload.php?page=' . self::ADMIN_PAGE_SLUG ) . '&amp;noheader=1" method="post" id="mla-filter">' . "\r\n";
//			echo '<form action="' . admin_url( 'upload.php?page=' . self::ADMIN_PAGE_SLUG ) . '" method="get" id="mla-filter">' . "\r\n";
			...
			echo "<br /><a href=\"upload.php?page=mla-menu&noheader=true&action=export&cb_attachment=all\" class=\"button js-export\"><span class=\"icon-archive\"></span> Export All</a></form><!-- id=mla-filter -->\r\n";
	 ...
	 * @return	array	success/failure message and NULL content
	 */
	private static function _export_items( $media ) {
		// Get all the files.
		$files     = array();
		$filenames = array();
		...
		foreach ( (array) $media as $att_id ) {
			...
			if ( !current_user_can( 'edit_post', $att_id ) )
				...
				if ( file_exists( $file_path ) ) {
					...
					// Get Filename 
					$test_filename = $filename = strrchr( $file_path, '/' ) ? substr( strrchr( $file_path, '/' ), 1) : $file_path;
				  
					// Get Extension
					$ext = strrchr( $filename, '.' ) ? strrchr( $filename, '.' ) : '';
				  
					$i = 1; // Check to make sure we don't already have this file
					while ( in_array( $test_filename, $filenames ) && $i < 50 ) {
				    
						// Regen Filename
						$test_filename = substr( $filename, 0, -1 * strlen( $ext ) ) . "({$i})" . $ext;
							$i++; 
		...
		$zip_obj->open( $zip_file, ZipArchive::OVERWRITE );
		$zip_obj->addEmptyDir( $zip_dir );
			
		// Stuff with content
		foreach ( $files as $i => $file ) {
			$zip_obj->addFile( $file, $zip_dir . '/' . $filenames[$i] );
		}
	...
	public static function regenerate_interface() {
		global $wpdb; ?>
...
?>
	...
	public static function ajax_process_image() {
		...
		if ( ! current_user_can( 'regenerate-thumbnails' ) )
	...
	function die_json_error_msg( $post_id, $message ) {
		die( json_encode( array( 'error' => sprintf( __( '&quot;%1$s&quot; (ID %2$s) failed to resize. The error message was: %3$s', 'regenerate-thumbnails' ), esc_html( get_the_title( $post_id ) ), $post_id, $message ) ) ) );
	}
