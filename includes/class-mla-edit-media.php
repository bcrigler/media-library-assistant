...
class MLAEdit {
	...
	public static function mla_add_meta_boxes_action( $post_type = 'unknown', $post = NULL ) {
		...
		if ( 'attachment' == $post_type ) {
			...
			add_meta_box( 'mla-thumb-override', 'Thumbnail Override', 'MLAEdit::mla_thumb_override_handler', 'attachment', 'side', 'low' );
	 ...
	 * @return	void	echoes the HTML markup for the meta box content
	 */
	public static function mla_thumb_override_handler($post) {
		
		global $post;
		
		$override_key = 'thumb_override';
	  	$thumb_meta = get_post_meta( $post->ID, $override_key, true );
	  	
	  	if( $thumb_meta ) { 
            echo '<img src="' . $thumb_meta . '" width="150" height="150" />';?>
            <input id="upload_image" type="text" size="28" name="thumb_meta" value="<?php echo $thumb_meta;?>" />
            <input id="upload_image_button" class="button" type="button" value="Edit Image" />
        <?php } else { ?>
            <input id="upload_image" type="text" size="28" name="thumb_meta" value="" /> 
            <input id="upload_image_button" class="button" type="button" value="Upload Image" />
        <?php } ?>
   		<p>Enter a URL or upload an image to override the thumbnail.  Used Primarily for Video/Audio/Miscellaneous and non-image files.</p>
	<?php
	...
	public static function mla_edit_attachment_action( $post_ID ) {
		...
		$override_key = 'thumb_override';
	  	$thumb_meta = get_post_meta( $post_ID, $override_key, true );
		
		if ( isset( $_POST['thumb_meta'] ) ) {
	        
	        if ( $thumb_meta == '' ) {
				add_post_meta( $post_ID, $override_key, $_POST['thumb_meta'] );
			} else {
				update_post_meta( $post_ID, $override_key, $_POST['thumb_meta'] );
			}
    	}
