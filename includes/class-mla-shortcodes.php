	...
	public static function mla_gallery_shortcode( $attr, $content = NULL ) {
		...
		foreach ( $attachments as $id => $attachment ) {
			...
			$override_key = 'thumb_override';
	  		$thumb_meta = get_post_meta( $attachment->ID, $override_key, true );
	  		$thumbdata = wp_get_attachment_metadata( $attachment->ID );
	  	
	  		if ( $thumb_meta ) {
	  			$item_values['filelink'] = '<a href="' . wp_get_attachment_url( $attachment->ID ) . '"><img width="150" height="150" src="' . $thumb_meta . '" class="attachment-thumbnail" alt="' . $item_values['caption'] . '" /></a>';
